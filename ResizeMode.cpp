	...
	if (unsigned char* inputData = stbi_load(filepath.string().c_str(), &inputWidth, &inputHeight, &inputNumComp, numCompReq)) {
		...
		if (resizeResult == 1) {
			...
			if (extension == ".jpg") {
				...
				writeResult = stbi_write_png(filepath.string().c_str(), newWidth, newHeight, numCompReq, outputData.data(), 50);
			...
			if (writeResult == 0) {
				...
				std::cout << GetModeName() << "Erro ao escrever o arquivo" << filepath << std::endl;
